...
export const mapIdList = (idList: ReadonlyArray<any>) =>
  (idList || [])  // If idList is undefined or null, default to an empty array
    .filter((id: any) => id !== '')  // Filter out empty strings
    .map((id: any) => ({ id }));     // Map the remaining ids to objects
